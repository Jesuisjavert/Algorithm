...
for tc in range(1,T+1):
    ...
    result=LinkedList()   ### result를 LinkedList의 인스턴스로 설정해준다.
    for i in range(N):
        result.append(arr[0][i])   ## 가장 첫줄을 append를 통해 넣어준다.
    row=1    ### 우리가 탐색하고 싶은 행을 말해준다. 처음 탐색할곳은 1이니 row는 1이다.
    while row<M:
        larger_index=result.find_lager_index(arr[row][0])   ### find_larger_index는 들어온값보다 큰 값이 있는 index를 반환해준다.
        if larger_index==-1:                ### 그 값이 -1이라면 result내에 큰값이 없는것이므로, 
            for i in range(N):              ### 가장 뒤에 해당리스트를 추가해준다.
                result.append(arr[row][i])
        else:                                ### 그 외에는 큰값이 존재하는 것이므로,
            for i in range(N-1,-1,-1):
                result.insert(larger_index,arr[row][i])   ## 그 index에 값을 하나씩 추가해준다.
        row+=1   ## 그 작업이 끝나면,  다음행을 탐색해야하므로, row를 +1 해준다.
    print('#{} '.format(tc),end='')
    total_len=result.sizeofList()   ### sizeofList 메소드를 통해 전체 크기를 구한다.
    result=reversed(result.slicing(total_len-10,total_len))   #### 끝에서 10개만 출력해줘야하므로 전체크기에서 -10에서 전체크기까지 슬라잇이해주고 reverse 해준다.
    print(*result)
